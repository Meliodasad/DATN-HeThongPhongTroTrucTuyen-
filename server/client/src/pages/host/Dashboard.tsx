...
const Dashboard = () => {
  ...
  const fetchData = async (startDate = fromDate, endDate = toDate) => {
    ...
    try {
      ...
      const statsRes = await hostService.hostStatic(params);
  ...
  // Calculate totals
  const totalRooms = data.kpis.roomsByStatus.reduce((sum, item) => sum + item.count, 0);
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg">
                ...
                <span className="text-lg font-semibold text-gray-900">
                  {totalRooms > 0 ? ((data.kpis.roomsByStatus.find(r => r._id === 'rented')?.count || 0) / totalRooms * 100).toFixed(1) : 0}%
                </span>
              </div>
